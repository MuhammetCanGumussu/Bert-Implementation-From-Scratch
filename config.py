...
def _parse_argsv2(cfg_classes: Tuple[Type]) -> List[argparse.Namespace]:
    ...
    ## let's seperate args relative to each config class
    #for cfg_class in cfg_classes:
    #    for key in vars(all_args).keys():
    #        if key in cfg_class.__dataclass_fields__.keys() and cfg_class.__name__.startswith("PreTrain"):
    #            setattr(pretrain_args, key, getattr(all_args, key))
    #        elif key in cfg_class.__dataclass_fields__.keys() and cfg_class.__name__.startswith("Model"):
    #            setattr(model_args, key, getattr(all_args, key))
...
#def _control_if_values_diffrent(scenario_name:str, dict_to_control:dict[str, Any], args:argparse.Namespace):
#    """if user gives None, then default value will be used and if user gives different value then raise error"""
#    for k, v in dict_to_control.items():
#        if args.__dict__[k] is None:
#            args.__dict__[k] = v
#        elif args.__dict__[k] != v:
#            raise argparse.ArgumentTypeError(f"{scenario_name}: ", f"you should not give {k} arg to this value {args.__dict__[k]}... (the value should be {v})")



#def _override_cfg_by_args(cfg:Type[dataclass], args:argparse.Namespace):
#    """overrides cfg by args"""
#    for field in cfg.__dataclass_fields__.values():
#        if args.__dict__[field.name] is not None:
#            setattr(cfg, field.name, args.__dict__[field.name])
#    return cfg
...
def _control_args(user_can_give_pretrain:List[str],
    ...
    #if type(default_values_model) != dict and type(default_values_model) == BertConfig:
    #    default_values_model = asdict(default_values_model)
    #if type(default_values_pretrain) != dict and type(default_values_pretrain) == PreTrainBertConfig:
    #    default_values_pretrain = asdict(default_values_pretrain)
...
def get_pretrain_bert_py_configs(verbose:bool=True) -> Tuple[BertConfig, dict[str, Any]]:
    ...
    if pretrain_args.do_eval_from_best_ckpt:
        ...
        # fix args, they are not can be given by user but they are fixed
        fixed_scenario_args = {
        ...
        ## controls if user give wrong args (cannot accept any args rather than can_be_given_keys)
        #_control_if_user_give_extra_args(can_be_given_keys=user_can_give, args=pretrain_args)   
#
        ## apply default values if user gives None
        #_assign_dict_to_args(dict_to_assign=default_values, args=pretrain_args, fixed_scenario_args=fixed_scenario_args)
#
        ### apply fix args
        #_assign_dict_to_args(dict_to_assign=fixed_scenario_args, args=pretrain_args)
#
        ## we cannot give any args for model cfg (we will use best ckpt model cfg unchangebly)
        #_control_if_user_give_extra_args(can_be_given_keys=[], args=model_args)
#
        ## we should override model_args by ckpt_model_config
        #_assign_dict_to_args(dict_to_assign=ckpt_model_config, args=model_args)
#
        ## general controls
        #_control_consistency(model_args=model_args, pretrain_args=pretrain_args)
#
...
#
        ## apply default values if user gives None
        #_assign_dict_to_args(dict_to_assign=default_values, args=pretrain_args)   
#
        ## apply fix args
        #_assign_dict_to_args(dict_to_assign=fixed_scenario_args, args=pretrain_args)
#
        ## we cannot give any args for model cfg (we will use hf model cfg unchangebly)
        #_control_if_user_give_extra_args(can_be_given_keys=[], args=model_args)
#
        ## we should use override model_args by hf model cfg
        #_assign_dict_to_args(dict_to_assign=asdict(BertConfig()), args=model_args)
#
        ## general controls
        #_control_consistency(model_args=model_args, pretrain_args=pretrain_args)
...
#
        ## we should use override model_args by hf model cfg
        #_assign_dict_to_args(dict_to_assign=model_config, args=model_args)
#
        ## general controls
        #_control_consistency(model_args=model_args, pretrain_args=pretrain_args)
...
#
        ...
        #_assign_dict_to_args(dict_to_assign=fixed_scenario_args, args=pretrain_args)
#
        ## we can give any args for model cfg (from scratch not hf weights)
        #_control_if_user_give_extra_args(can_be_given_keys=asdict(BertConfig()).keys(), args=model_args)
#
        ## we should use override model_args by hf model cfg
        #_assign_dict_to_args(dict_to_assign=model_config, args=model_args)
#
